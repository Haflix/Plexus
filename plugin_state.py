...
mutation happens through PluginCore._transition_plugin(name, new_state).
External code reads via pc.plugin_states[name].state and
pc.plugin_states[name].last_errors but must NOT mutate fields directly —
no setters are exposed.

Iteration contract: external code reading `pc.plugin_states` MUST snapshot
...
`RuntimeError: dictionary changed size during iteration`. Snapshot pattern:
    ...
    states_snapshot = dict(pc.plugin_states)
