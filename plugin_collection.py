...
from utils import LogUtil, Request, MQTTLogHandler
...
class PluginCollection(object):
    ...
    def __init__(
        ...
        self._logger.addHandler(MQTTLogHandler())
    ...
    def _start_mqtt_broker(self):
        ...
        async def start():
            self.broker = Broker(config={
                'listeners': {
                    f'tcp://{self.mqtt_broker_ip}:{self.mqtt_port}': {}
                }
            })
            await self.broker.start()

        self.broker_task = asyncio.create_task(start())

    def _init_mqtt_client(self):
        """Initialize MQTT client for all instances"""
        self.mqtt_client = mqtt.Client(client_id=self.hostname)
        self.mqtt_client.tls_set() #NOTE?: Do i need to add something???? 
        self.mqtt_client.connect(self.mqtt_broker_ip, self.mqtt_port)
        self.mqtt_client.loop_start()
        
        # Subscribe to request topic
        self.mqtt_client.subscribe(f"requests/{self.hostname}/#")
        self.mqtt_client.message_callback_add("requests/+/+", self._handle_mqtt_request)
    ...
    def reload_plugins(self):
        ...
        self.start_loops()
        # In PluginCollection.reload_plugins()
            ...
            if plugin.loop_req and plugin.loop_running == False:
                try:
                    self.thread_list.append(
                        threading.Thread(
                            target=plugin.comm_layer,
                            kwargs={"function": plugin.loop_start},
                            daemon=True,
                        ).start()
                    )
                except Exception as e:
                    self._logger.error(
                        f"Error starting loop of '{plugin.plugin_name}': {e}"
                    )
    ...
    def create_request(self, author: str, target: str, args, timeout=None) -> Request:
        ...
        request = Request(
            self.hostname, author, target, args, self.request_lock, timeout
        )
        with self.request_lock:
            self.requests[request.id] = request
        self._logger.info(
            f"Request {request.id} created by {author} targeting {target} with args ({args})"
        )

        # Start thread for the processing of the request
        self.thread_list.append(
            threading.Thread(
                target=self._process_request, args=(request,), daemon=True
            ).start()
        )
    ...
    def create_request_wait(self, author, target, args, timeout=None) -> Request:
        """Create request with pickle serialization"""
        request = Request(
            self.hostname, author, target, args, 
            self.request_lock, timeout
        )
        
        if '@' in target:  # Remote request
            target_host, target_plugin = target.split('@', 1)
            request.target = target_plugin
            self.mqtt_client.publish(
                f"requests/{target_host}",
    ...
    def _process_request(self, request: Request):
        ...
        plugin: Plugin = next(
            (p for p in self.plugins if p.plugin_name == plugin_name), None
        )
        ...
        try:
            ...
            self._logger.error(
                f"Error processing request from {request.author} to {request.target}({request.args}) ({request.id}): {e}"
            )
            request.set_result(str(e), error=True)

    def _handle_mqtt_request(self, client, userdata, msg):
        """Handle incoming MQTT requests"""
        try:
            request = pickle.loads(msg.payload)
            request._Request__request_lock = self.request_lock  # Use host's lock
            self._process_request(request)
        except Exception as e:
            self._logger.error(f"Error handling MQTT request: {e}")
    ...
    async def loop(self):
        ...
        while True:
            ...
            await asyncio.get_event_loop().create_task(self.cleanup_requests())
        ...
        try:
            if self.asynced:
                if asyncio.iscoroutinefunction(function):  # Check if function is async
                    ...
                    self._logger.error(
                        f"Function {function.__name__} is not async but called in async mode!"
                    )
